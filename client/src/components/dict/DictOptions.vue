<template>
  <div class="dict-options">
    <VueDraggable class="options" v-model="options" item-key="id" handle=".drag">
      <template #item="{ index }">
        <div class="option">
          <div class="drag" v-show="editable">:::</div>
          <div class="label">
            <el-input
          ...
          <div class="remove" v-show="editable" :class="{ empty: options.length < 2 }">
            <el-icon @click="remove(index)"><Remove /></el-icon>
          </div>
        </div>
      </template>
    </VueDraggable>
    <div v-show="editable" class="add" @click="add">
...
const inputWidth = computed(() => (props.editable ? '250px' : '270px'))
